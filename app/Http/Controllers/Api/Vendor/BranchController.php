...
class BranchController extends Controller
{
    ...
    /**
     * Get all branches for the authenticated vendor's store
     */
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $query = Branch::where('store_id', $vendor->store_id);
    ...
    public function show($id): JsonResponse
    {
        try {
            ...
            $branch = Branch::where('store_id', $vendor->store_id)
                ->find($id);
    ...
    /**
     * Create a new branch for the vendor's store
     */
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            if ($request->boolean('is_main', false)) {
                Branch::where('store_id', $vendor->store_id)
                    ->update(['is_main' => false]);
            }

            // If no main branch exists for this store, make this one main
            $hasMainBranch = Branch::where('store_id', $vendor->store_id)
                ->where('is_main', true)
                ->exists();
    ...
    public function update(Request $request, $id): JsonResponse
    {
        try {
            ...
            if ($request->has('is_main') && $request->boolean('is_main')) {
                ...
                $otherMainBranches = Branch::where('store_id', $vendor->store_id)
                    ->where('id', '!=', $branch->id)
                    ->where('is_main', true)
                    ->exists();
    ...
    public function destroy($id): JsonResponse
    {
        try {
            ...
            $branchCount = Branch::where('store_id', $vendor->store_id)->count();
            if ($branchCount <= 1) {
            ...
            if ($branch->is_main) {
                $newMainBranch = Branch::where('store_id', $vendor->store_id)
                    ->where('id', '!=', $branch->id)
                    ->first();
    ...
    /**
     * Get main branch for the vendor's store
     */
