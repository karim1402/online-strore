...
Route::prefix('user')->group(function () {
    ...
    Route::middleware('auth:api')->group(function () {
        ...
        Route::controller(OrderController::class)->prefix('orders')->group(function () {
            ...
            Route::post('/{orderId}/chat', [\App\Http\Controllers\Api\User\ChatController::class, 'sendMessage'])->name('v2.user.orders.chat');
        });
        ...
        // Vouchers
        Route::post('vouchers/verify', [\App\Http\Controllers\Api\User\VoucherController::class, 'verify'])->name('v2.user.vouchers.verify');

        // Payments (protected)
        Route::post('payments/create-intention', [\App\Http\Controllers\Api\User\PaymentController::class, 'createIntention'])->name('v2.user.payments.create-intention');
    });
    ...
    Route::controller(ModuleController::class)->prefix('modules')->group(function () {
        ...
        Route::get('/{id}/ads', [\App\Http\Controllers\Api\User\ModuleAdController::class, 'getByModule'])->name('v2.user.modules.ads');
    });
    ...
    // Home Ads
    Route::get('home-ads',              [\App\Http\Controllers\Api\User\HomeAdController::class, 'index'])->name('v2.user.home-ads.index');
    Route::get('home-ads/{id}/products', [\App\Http\Controllers\Api\User\HomeAdController::class, 'showProducts'])->name('v2.user.home-ads.products');

    // Service Area check
    Route::post('check-service-area', [AddressController::class, 'checkServiceArea'])->name('v2.user.check-service-area');

    // Splash Ad
    Route::get('splash-ad', [\App\Http\Controllers\Api\User\SplashAdController::class, 'getRandom'])->name('v2.user.splash-ad.random');

    // Payment webhook (public)
    Route::post('payments/webhook', [\App\Http\Controllers\Api\User\PaymentController::class, 'webhook'])->name('v2.user.payments.webhook');

    // Working Hours
    Route::get('working-hours', [\App\Http\Controllers\Api\User\WorkingHoursController::class, 'index'])->name('v2.user.working-hours');

    // App Settings
    Route::get('settings',    [\App\Http\Controllers\Api\User\SettingsController::class, 'index'])->name('v2.user.settings');
    Route::get('app-version', [\App\Http\Controllers\Api\User\SettingsController::class, 'appVersion'])->name('v2.user.app-version');
